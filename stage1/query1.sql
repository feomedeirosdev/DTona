WITH
  ...
  SELECT t2.seller_id AS "Seller ID",
    ...
    sum(
      (CASE WHEN julianday(order_delivered_customer_date) > julianday(order_estimated_delivery_date) THEN 1 ELSE 0 END) / count(DISTINCT t2.order_id)
      ) AS "Delay (%)"
...
SELECT
  t1.*,
  t2.seller_state AS "Seller State",
  t2.seller_city AS "Seller City"

FROM tb_sellers_semi_complete AS t1

LEFT JOIN tb_sellers AS t2 ON t1."Seller ID" = t2.seller_id
